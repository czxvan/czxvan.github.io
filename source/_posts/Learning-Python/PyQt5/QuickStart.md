...
```python
import sys
from PyQt5.QtWidgets import QApplication, QTreeView, QVBoxLayout, QMainWindow, QWidget, QFileSystemModel
...
class FileExplorer(QMainWindow):
    def __init__(self):
        ...
        tree_view.setRootIndex(model.index(QDir.rootPath()))
...
![11-file-system-model](../../pic/Learning-Python/pyqt5/11-file-system-model.png)
...
![12-checkbale-file-system-model](../../pic/Learning-Python/pyqt5/12-checkbale-file-system-model.png)
...
class SimpleTable(QMainWindow):
    def __init__(self):
        ...
        table.setHorizontalHeaderLabels(['Name', 'Age', 'Country'])
...
![13-QTableWidget](../../pic/Learning-Python/pyqt5/13-QTableWidget.png)
...
def onItemSelectionChanged(self):
    ...
    columns = sorted(set(index.column() for index in self.selectedIndexes()))
    row_all = self.rowCount()
    column_all = self.columnCount()
    # Construct text for selection area
    text = "；已选中"
    row_count = len(rows)
    column_count = len(columns)
    if row_count == 1 and column_count == 1:
        text = text + "第" + str(rows[0] + 1) + "行，第" + str(columns[0] + 1) + "列"
    elif row_count == 1 and column_count == column_all:
        text = text + "第" + str(rows[0] + 1) + "行"
    elif row_count == row_all and column_count == 1:
        text = text + "第" + str(columns[0] + 1) + "列"
    elif row_count == row_all and column_count == column_all:
        text = text + "全部"
    else:
        text = text + str(row_count) + "行，" + str(column_count) + "列"
        print(text)
```
...
def handleCellChanged(self, row, column):
    ...
    if item is not None:
        text = item.text()
        print(f"Cell ({row}, {column}) is changed to: {text}")
```
